...
GENTLE_MODE_GUIDANCE = {
    "natal": "生まれもった考え方や、元気が出やすい場面を見る紙です。",
    "progressed": "最近の心の変わり方と、今の自分に合う進み方を見る紙です。",
    "transit": "今の流れと、外から来やすい出来事のテーマを見る紙です。",
    "triple": "生まれもった土台、今の心、今の流れをまとめて見る紙です。",
    "synastry": "ふたりが楽になりやすい関わり方を見る紙です。",
}

GENTLE_MODE_FOCUS = {
    "natal": "『その人らしさ』と『どんな場面で力が出やすいか』を見ると分かりやすいです。",
    "progressed": "『前より変わってきた所』と『無理を減らすコツ』を見ると分かりやすいです。",
    "transit": "『今ふれやすいテーマ』と『落ち着いて動くコツ』を見ると分かりやすいです。",
    "triple": "『変えなくていい所』と『今だけ調整したい所』を分けて見ると分かりやすいです。",
    "synastry": "『安心しやすい点』と『すれ違いを減らすコツ』を見ると分かりやすいです。",
}

PLANET_GENTLE_LABELS = {
    "太陽": "太陽（その人のまんなか）",
    "月": "月（こころ）",
    "水星": "水星（考え方と言葉）",
    "金星": "金星（好きなものと心地よさ）",
...
PLANET_GENTLE_SENTENCE_TEMPLATES = {
    "太陽": "{label}: {theme}の場面で、その人らしさが出やすいです。",
    "月": "{label}: {theme}が、心の元気につながりやすいです。",
    "水星": "{label}: {theme}で、考えや言葉が動きやすいです。",
    "金星": "{label}: {theme}に、心地よさを感じやすいです。",
    "火星": "{label}: {theme}で、動く力が出やすいです。",
    "木星": "{label}: {theme}は、育ちやすく広がりやすいです。",
    "土星": "{label}: {theme}は、時間をかけるほど力になりやすいです。",
}

GENTLE_TERM_GLOSSARY = {
    "太陽": "その人のまんなか。『自分らしさ』の土台です。",
    "月": "こころ。安心しやすさや気分の動きに関係します。",
    "水星": "考え方と言葉。話し方や学び方に関係します。",
    "金星": "好きなものと心地よさ。楽しみ方や親しみ方に関係します。",
    "火星": "動く力。やる気や進み方に関係します。",
    "木星": "広がりやすい所。育ちやすいテーマに関係します。",
    "土星": "がんばりどころ。時間をかけて育てる所に関係します。",
    "アスペクト": "星どうしの組み合わせ。力の混ざり方を見る言葉です。",
    "相性": "ふたりのかみあわせ。楽な所と工夫がいる所を見ます。",
}

GENTLE_TEXT_REPLACEMENTS = (
    ("パートナーシップ", "人とのつながり"),
    ("対人調整", "人との調整"),
    ("キャリア", "仕事"),
    ("社会評価", "仕事でどう見られるか"),
...
def _gentle_text(text: str | None) -> str:
    normalized = unicodedata.normalize("NFKC", str(text or ""))
    normalized = re.sub(r"\s+", " ", normalized).strip()
    if not normalized:
        return ""
    for source, target in GENTLE_TEXT_REPLACEMENTS:
...
def _gentle_list_text(text: str | None, *, limit: int = 2) -> str:
    normalized = _gentle_text(text)
    if not normalized:
        return ""
    if "、" in normalized:
        items = [part.strip() for part in normalized.split("、") if part.strip()]
    else:
        items = [_first_sentence(normalized, "")]
    selected = items[:limit]
    if not selected:
        return ""
    if len(selected) == 1:
        return selected[0]
    return f"{'、'.join(selected[:-1])}や{selected[-1]}"


def _gentle_tip_text(text: str | None) -> str:
    normalized = _gentle_text(text)
    if not normalized:
        return ""
    if "、" in normalized:
        ...
        snippet = _first_sentence(normalized, normalized)
    return _ensure_sentence(snippet)


def _gentle_placement_line(item: dict[str, str]) -> str:
    planet = str(item.get("planet", "") or "")
    label = PLANET_GENTLE_LABELS.get(planet, planet or "大事なテーマ")
    theme = _gentle_list_text(item.get("life_manifestation"), limit=2)
    if theme:
        template = PLANET_GENTLE_SENTENCE_TEMPLATES.get(planet, "{label}: {theme}の場面で、力が出やすいです。")
        return template.format(label=label, theme=theme)
    mood = _gentle_text(_first_sentence(item.get("psychological_pattern"), ""))
    if mood:
        return f"{label}: {mood}が出やすいです。"
    return f"{label}: このテーマは、その人の中で大事な場所になりやすいです。"


def _simplify_aspect_identity(text: str | None) -> str:
    normalized = unicodedata.normalize("NFKC", str(text or "")).strip()
    if not normalized:
        return "星どうしの組み合わせ"
    match = re.match(r"(.+?と.+?)の.+", normalized)
    if match:
        return f"{match.group(1)}の組み合わせ"
    return normalized


def _gentle_glossary_terms(
    mode: str,
    placements: list[dict[str, str]],
    aspects: list[dict[str, str]],
) -> list[tuple[str, str]]:
    ...
    for item in placements[:4]:
        planet = str(item.get("planet", "") or "")
        if planet and planet in GENTLE_TERM_GLOSSARY and planet not in seen:
            terms.append((planet, GENTLE_TERM_GLOSSARY[planet]))
            seen.add(planet)
    if aspects and "アスペクト" not in seen:
        terms.append(("アスペクト", GENTLE_TERM_GLOSSARY["アスペクト"]))
        seen.add("アスペクト")
    if mode == "synastry" and "相性" not in seen:
        terms.append(("相性", GENTLE_TERM_GLOSSARY["相性"]))
    return terms


def generate_gentle_report(
    *,
    mode: str,
    subject_label: str,
    placements: list[dict[str, str]],
    ...
    lines = [
        ...
        f"対象: {subject_label}",
        f"読み方: {mode_label}",
        "この紙は、むずかしい占星術の言葉をできるだけへらした説明です。",
        "良い悪いを決める紙ではありません。",
        f"この紙で見ること: {GENTLE_MODE_GUIDANCE.get(mode, '今の大事なテーマを見る紙です。')}",
        f"読み方のコツ: {GENTLE_MODE_FOCUS.get(mode, '1つでも「これなら分かる」があれば十分です。')}",
        "=" * 60,
        "",
        "【まず知っておけば十分なこと】",
        "- ぜんぶを一度に分からなくて大丈夫です。",
        "- 1つでも『これならできそう』が見つかれば十分です。",
        "- 小さな子に読むときは、『この人はこうすると楽になりやすいんだね』という読み方で大丈夫です。",
    ]
    if placements:
        lines.extend(["", "【大事なポイント】"])
        for item in placements[:3]:
            lines.append(f"- {_gentle_placement_line(item)}")
            tip = _gentle_tip_text(item.get("practical_advice"))
            if tip:
                lines.append(f"  ためしてみること: {tip}")
    if aspects:
        lines.extend(["", "【こまったときのヒント】"])
        for item in aspects[:2]:
            guidance = _gentle_tip_text(item.get("practical_guidance"))
            if guidance:
                lines.append(f"- {_simplify_aspect_identity(item.get('identity'))}: {guidance}")
    overlay_tips = []
    for item in (overlays or [])[:2]:
        tip = _gentle_tip_text(item.get("practical_advice"))
        if tip and tip not in overlay_tips:
            overlay_tips.append(tip)
    if overlay_tips:
        lines.extend(["", "【ふたりで楽になるコツ】"])
        for tip in overlay_tips:
            lines.append(f"- {tip}")
    lines.extend(["", "【言葉の短い説明】"])
    for term, description in _gentle_glossary_terms(mode, placements, aspects):
        lines.append(f"- {term}: {description}")
    lines.extend(
        [
            "",
            "【さいごに】",
            "- これは未来をこわがるための紙ではありません。",
            "- 今日できる小さなことを1つ試せば十分です。",
        ]
    )
    return "\n".join(lines)
...
def _save_report_files(
    ...
    *,
    easy_report_text: str | None = None,
    ...
    if easy_report_text is not None:
        easy_report_path = output_dir / "astrology_easy_report.txt"
        easy_report_path.write_text(easy_report_text, encoding="utf-8")
        payload["easy_report_path"] = str(easy_report_path)
...
def run_natal_report(target: dict, *, person_name: str = "あなた", include_minor_aspects: bool = True, include_composite_aspects: bool = True) -> dict:
    ...
    easy_report_text = generate_gentle_report(
        mode="natal",
        subject_label=person_name,
        placements=placement_interpretations,
        aspects=aspect_interpretations,
...
def main() -> None:
    ...
    if result.get("easy_report_path"):
        print(f"[SAVE] easy report: {result['easy_report_path']}")
