...
def _render_page(request: Request, form_values: dict, report_payload: dict | None = None, error_message: str | None = None) -> HTMLResponse:
    ...
    context = {
        ...
        "easy_report_title": f"{mode_label}のだれでも読めるやさしい版",
        "easy_report_description": "専門用語をできるだけ減らし、前提知識ゼロでも読みやすくした版です。高齢の方や子どもに説明するときの土台にも使えます。",
        ...
        "raw_chart_title": f"{mode_label}のプロ向け生データ",
        "raw_chart_description": "惑星・度数・ハウス・アスペクトを解釈なしで並べた原文です。占星術経験者や、プロ向けに調整した LLM へ渡す土台に向いています。",
        "llm_prompt_title": f"{mode_label}の LLM読み解き用プロンプト",
        "llm_prompt_description": "外部の要約補助や読み替えに渡すための、日本語前提の指示文です。",
        "compact_data_title": f"{mode_label}の圧縮データ",
        "compact_data_description": "LLM や外部処理に渡しやすいよう、要点を短く JSON 化したデータです。",
        "easy_report_text": report_payload.get("easy_report_text", "") if report_payload else "",
        ...
        "raw_chart_text": report_payload.get("raw_chart_text", "") if report_payload else "",
        "llm_prompt_text": report_payload.get("llm_prompt_text", "") if report_payload else "",
        "compact_data_text": json.dumps(report_payload.get("compact_data", {}), ensure_ascii=False, indent=2) if report_payload and report_payload.get("compact_data") else "",
        "easy_report_path": report_payload.get("easy_report_path", "") if report_payload else "",
        ...
        "llm_prompt_path": report_payload.get("llm_prompt_path", "") if report_payload else "",
        "compact_data_path": report_payload.get("compact_data_path", "") if report_payload else "",
        ...
        return JSONResponse(
            {
                ...
                "easy_report_text": payload.get("easy_report_text", ""),
                "result_text": payload.get("result_text", ""),
                "raw_chart_text": payload.get("raw_chart_text", ""),
                "llm_prompt_text": payload.get("llm_prompt_text", ""),
                "compact_data": payload.get("compact_data"),
                "easy_report_path": payload.get("easy_report_path"),
                "result_path": payload.get("result_path"),
                "interpretation_path": payload.get("interpretation_path"),
                "llm_prompt_path": payload.get("llm_prompt_path"),
                "compact_data_path": payload.get("compact_data_path"),
